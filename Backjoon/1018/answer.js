...
for (let i = 0; i <= N - 8; i++) {
  for (let j = 0; j <= M - 8; j++) {
    const subBoard = chessBoard
      .slice(i, i + 8)
      .map((row) => row.slice(j, j + 8).split(""));
    ...
    if (min > Math.min(countDiffWithBBoard, countDiffWithWBoard))
      min = Math.min(countDiffWithBBoard, countDiffWithWBoard);
  }
}
